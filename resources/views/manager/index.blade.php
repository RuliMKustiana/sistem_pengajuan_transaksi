...
@section('title', 'Persetujuan Manager')
@section('page_heading', 'Antrean Verifikasi Manager')
...
<div class="card card-custom border-0 shadow-sm">
    <div class="card-header bg-white pt-4 pb-2 border-bottom-0 px-4">
        <h5 class="fw-bold text-dark mb-1">Daftar Pengajuan Menunggu Approval Manager</h5>
        <p class="small text-muted mb-0">Tinjau parameter anggaran operasional travel secara teliti sebelum memberikan otorisasi tingkat menengah.</p>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="bg-secondary bg-opacity-10 text-secondary rounded-circle d-flex align-items-center justify-content-center fw-bold text-uppercase" style="width: 28px; height: 28px; font-size: 0.7rem;">
                            ...
                            <td>
                                <span class="badge bg-light text-dark border rounded-pill px-2 py-1 mb-1 fw-medium" style="font-size: 0.7rem;">
                                    {{ $submission->category->name }}
                                </span>
                                <p class="text-muted mb-0 text-truncate" style="max-width: 220px; font-size: 0.8rem;" title="{{ $submission->description }}">
                            ...
                            <td>
                                @if($submission->attachment)
                                    <a href="{{ asset('storage/' . $submission->attachment) }}" target="_blank" class="btn btn-sm btn-light border rounded-2 px-2 py-1 text-secondary shadow-none" style="font-size: 0.75rem;">
                            ...
                            <td class="pe-4 py-3">
                                <form action="{{ route('manager.process', $submission->id) }}" method="POST">
                                    @csrf
                                    <!-- Input Catatan Review Ringkas -->
                                    <div class="mb-2">
                                        <input type="text" name="notes" class="form-control form-control-sm form-control-notes rounded-2" 
                                               style="font-size: 0.75rem;" placeholder="Tulis catatan peninjauan...">
                                    </div>
                                    <!-- Tombol Pilihan Opsi Setuju / Tolak -->
                                    <div class="d-flex gap-2">
                                        <button type="submit" name="action" value="approve" class="btn btn-sm btn-success flex-grow-1 rounded-2 fw-semibold shadow-sm py-1">
                                            <i class="fa-solid fa-check small me-1"></i> Setuju
                                        </button>
                                        <button type="submit" name="action" value="reject" class="btn btn-sm btn-danger flex-grow-1 rounded-2 fw-semibold shadow-sm py-1">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">
                                <div class="mb-2 fs-3 opacity-50"><i class="fa-solid fa-envelope-open-text text-muted"></i></div>
                                <p class="mb-0 small fw-medium">Tidak ada antrean dokumen transaksi yang memerlukan persetujuan Manager.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
